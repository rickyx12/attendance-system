...
class Timelog_model extends CI_Model {
	...
	public function getSectionAttendanceByDate($section,$date) {

		$section1 = $this->db->escape_str($section);
		$date1 = $this->db->escape_str($date);

		$sql = "
		SELECT t.id,CONCAT_WS(' ',s.last_name,s.first_name,s.middle_name) as student,t.dateTap, t.timeTap, t.type, gl.schedule_timein, gl.schedule_timeout
		FROM grade_level gl
		INNER JOIN students s
		ON s.id = gl.student_id
		INNER JOIN settings_gradelevel sg
